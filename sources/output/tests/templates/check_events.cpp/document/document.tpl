...
/* Checkers */
static inline int check_event_function_entry(CTFReader::Event& event,
...
static inline int check_event_function_exit(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t func)
{
...
static inline int check_event_call_pre(CTFReader::Event& event,
...
static inline int check_event_call_post(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t func)
{
...
{
public:
	check_event_memory_events(CTFReader::Event& event,
...
static inline int check_event_locked_op(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
...
static inline int check_event_io_mem_op(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t addr, uint64_t size, enum kedr_memory_event_type type)
{
	if(check_tid(event, traceReader, tid)
...
static inline int check_event_memory_barrier(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
...
static inline int check_event_alloc(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
...
static inline int check_event_free(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t addr)
{
	if(check_tid(event, traceReader, tid)
...
static inline int check_event_lock(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
...
static inline int check_event_unlock(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t lock_id, enum kedr_lock_type type)
{
	std::string type_str;
...
static inline int check_event_signal(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
...
static inline int check_event_wait(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t obj_id, enum kedr_sw_object_type type)
{
	if(check_tid(event, traceReader, tid)
...
static inline int check_event_thread_create(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t child_tid)
{
	if(check_tid(event, traceReader, tid)
...
static inline int check_event_thread_join(CTFReader::Event& event,
	const KEDRTraceReader& traceReader, uint64_t tid, uint64_t pc,
	uint64_t child_tid)
{
	if(check_tid(event, traceReader, tid)
