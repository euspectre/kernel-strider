...
#define KEDR_HELPER_SCRIPT_PATH "/usr/local/bin/kedr_get_sections.sh"
/* ====================================================================== */
	...
	loff_t *f_pos)
{
	...
	{
		pr_err("[sample] debug_write_common: "
...
kedr_run_um_helper(char *target_name)
{
	...
	if (ret_status != 0) {
		pr_err("[sample] Failed to execute %s, status is 0x%x\n",
...
reset_section_subsystem(void)
{
	...
	if (mutex_lock_killable(&section_mutex) != 0)
	{
		pr_err("[sample] kedr_reset_section_subsystem: "
			"got a signal while trying to acquire a mutex.\n");
		return -EINTR;
	}
...
kedr_get_sections(struct module *mod)
{
	...
	if (mutex_lock_killable(&section_mutex) != 0)
	{
		pr_err("[sample] kedr_get_sections: "
			"got a signal while trying to acquire a mutex.\n");
