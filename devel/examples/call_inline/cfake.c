...
/* CFAKE_SAVE_REG_STATE
 * Save scratch registers and flags, this can be used before an inserted 
 * function call.
 * 
 * CFAKE_RESTORE_REG_STATE
 * Restore scratch registers and flags, this can be used after an inserted 
 * function call.
 * 
 * CFAKE_GET_ARGS2(arg1, arg2)
 * Get the values of the arguments directly from the registers they are 
 * passed in and store them in 'arg1' and 'arg2' ('arg1' and 'arg2' should 
 * be the names of the variables to store the values in, not the pointers to 
 * these variables).
 * This allows not to depend on any calling convention that the compiler 
 * uses in this particular case. Still we try to comply to the commonly used
 * convention. 
 * The arguments are assumed to be unsigned long values.
 */
#ifdef CONFIG_X86_64
# define CFAKE_SAVE_REG_STATE 	\
	"pushf\n\t"		\
	"push %%rax\n\t"	\
	"push %%rcx\n\t"	\
	"push %%rdx\n\t"	\
...
# define CFAKE_RESTORE_REG_STATE \
	"pop %%r11\n\t"		\
	"pop %%r10\n\t"		\
	"pop %%r9\n\t"		\
	"pop %%r8\n\t"		\
...
/* Note that "%rdi" and "%rsi" are added to the clobber list to prevent the 
 * compiler from using them for parameters %0 and %1. */
# define CFAKE_GET_ARGS2(arg1, arg2) { 		\
	unsigned long cfake_tmp_arg1_;		\
	unsigned long cfake_tmp_arg2_;		\
	asm volatile(	"mov %%rdi, %0\n\t"	\
			"mov %%rsi, %1\n\t"	\
			: "=g"(cfake_tmp_arg1_), "=g"(cfake_tmp_arg2_) \
			: /* no inputs */	\
			: "memory", "%rdi", "%rsi");	\
	arg1 = cfake_tmp_arg1_;			\
	arg2 = cfake_tmp_arg2_;			\
}
...
/* Note that "%eax" and "%edx" are added to the clobber list to prevent the 
 * compiler from using them for parameters %0 and %1. */
# define CFAKE_GET_ARGS2(arg1, arg2) {		\
	unsigned long cfake_tmp_arg1_;		\
	unsigned long cfake_tmp_arg2_;		\
...
unsigned long 
my_sensor_func(unsigned long first, unsigned long second)
{
	CFAKE_GET_ARGS2(first, second);
	
	printk("[DBG] Args: (%lx, %lx)\n",
		first, second);
	return second + 2;
}
...
//<>
#ifdef CONFIG_X86_64
	asm volatile(	CFAKE_SAVE_REG_STATE
			"mov $0x123456, %%rdi\n\t"
			"mov $0x987654, %%rsi\n\t"
			"call my_sensor_func\n\t"
			"mov %%rax, %%rbx\n\t"
			...
			: /* no inputs */
			: "memory", "%rbx" 
	/* We restore other clobbered regs manually; memory is changed when
	 * we execute 'push' instructions from CFAKE_SAVE_REG_STATE. */
	);
#else
	asm volatile(	CFAKE_SAVE_REG_STATE
			"mov $0x123456, %%eax\n\t"
