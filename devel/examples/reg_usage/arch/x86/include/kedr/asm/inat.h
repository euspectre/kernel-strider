...
#define INAT_USES_REG_DX	(1 << (INAT_USES_REG_OFFS + 2))
#define INAT_USES_REG_BX	(1 << (INAT_USES_REG_OFFS + 3))
#define INAT_USES_REG_SP	(1 << (INAT_USES_REG_OFFS + 4))
#define INAT_USES_REG_BP	(1 << (INAT_USES_REG_OFFS + 5))
#define INAT_USES_REG_SI	(1 << (INAT_USES_REG_OFFS + 6))
#define INAT_USES_REG_DI	(1 << (INAT_USES_REG_OFFS + 7))
#define INAT_USES_REG_BITS	8
#define INAT_USES_REG_MASK	(0xFF << (INAT_USES_REG_OFFS))

#if (INAT_USES_REG_OFFS + INAT_USES_REG_BITS > 32)
# error Not enough space in insn_attr_t left for register usage data
#endif
...
{
	attr &= ~INAT_USES_REG_MASK; /* just in case */
	return attr & ~INAT_GRP_MASK;
}
...
static inline int inat_reg_usage_attribute(insn_attr_t attr)
{
	return attr & INAT_USES_REG_MASK;
}
#endif
