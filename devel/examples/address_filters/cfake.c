...
cfake_open(struct inode *inode, struct file *filp)
{
	...
	//<>
	pr_info("[DBG] inode=%p (is_stack=%d, is_user=%d), filp=%p (is_stack=%d, is_user=%d)\n",
		inode, 
		is_stack_address((unsigned long)inode), 
		is_user_space_address((unsigned long)inode),
		filp,
		is_stack_address((unsigned long)filp), 
		is_user_space_address((unsigned long)filp));
	//<>
	...
	loff_t *f_pos)
{
	...
	//<>
	pr_info("[DBG] filp=%p (is_stack=%d, is_user=%d), buf=%p (is_stack=%d, is_user=%d)\n",
		filp,
		is_stack_address((unsigned long)filp), 
		is_user_space_address((unsigned long)filp),
		buf,
		is_stack_address((unsigned long)buf), 
		is_user_space_address((unsigned long)buf));
	//<>
	
	//<>
	pr_info("[DBG] f_pos=%p (is_stack=%d, is_user=%d), func=%p (is_stack=%d, is_user=%d)\n",
		f_pos,
		is_stack_address((unsigned long)f_pos), 
		is_user_space_address((unsigned long)f_pos),
		&cfake_write,
		is_stack_address((unsigned long)&cfake_write), 
		is_user_space_address((unsigned long)&cfake_write));
	//<>
	
	//<>
	pr_info("[DBG] &dev=%p (is_stack=%d, is_user=%d), dev=%p (is_stack=%d, is_user=%d)\n",
		&dev,
		is_stack_address((unsigned long)&dev), 
		is_user_space_address((unsigned long)&dev),
		dev,
		is_stack_address((unsigned long)dev), 
		is_user_space_address((unsigned long)dev));
	//<>
